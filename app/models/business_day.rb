...
class BusinessDay < ApplicationRecord
  ...
  def morning_started_cannot_be_greater_than_morning_ended
    errors.add(:morning_started_at, :less_than, count: morning_ended_at.strftime('%H:%M')) if morning_started_at.to_i > morning_ended_at.to_i && morning_ended_at.present?
  end

  def afternoon_started_cannot_be_greater_than_afternoon_ended
    errors.add(:afternoon_started_at, :less_than, count: afternoon_ended_at.strftime('%H:%M')) if afternoon_started_at.to_i > afternoon_ended_at.to_i && afternoon_ended_at.present?
  end
end
