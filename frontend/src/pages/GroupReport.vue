  ...
  <main-layout :title="$t('report.groupTitle', { name: group.name })">
    ...
    <div class="toolbar mt-5 clearfix">
      ...
      <div v-if="isValidTime" class="float-right">
        <button class="btn btn-success mx-2" @click="exportFile($event,{ type: 'zip', requestPath: `/groups/${$route.params.id}/report`, fileName: `report_${group.name}_${dateData.date}` })">{{ $t('button.group.exportZIPGroupReport') }}</button>
        <button class="btn btn-success" @click="exportFile($event, { type: 'csv', requestPath: `/groups/${$route.params.id}/report`, fileName: `report_${group.name}_${dateData.date}` })">{{ $t('button.group.exportCSVGroupReport') }}</button>
      </div>
    </div>

    <table v-if="isValidTime" class="table sortable-table bg-light mt-5">
      <thead>
        <th @click="sortBy('name')">{{ $t('tableHeader.name') }}
        ...
        <th @click="sortBy('working_hours')">{{ $t('tableHeader.working_hours') }}
