  ...
  <setting-layout sidebar-type="company" :title="$t('company.title')" :subtitle="$t('company.users.title')">
    ...
    <table class="table table-scroll bg-light mt-5">
      <thead>
        ...
        <th class="cell-mw-120">{{ $t('company.users.tableHeader.role') }}</th>
      ...
      <tbody>
        <tr v-for="user in filterByEmail(email)" :class="!user.activated ? 'rejected' : ''">
          ...
          <td>
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('company.users.tooltip.edit')" @click="toggleEditModal(user)" v-if="$auth('User', currentUser, user).canEdit()">
            ...
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('company.users.tooltip.deactivateUser')" @click="deactivateUser(user.id)" v-if="user.activated">
              <svg width="24px" height="24px" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor"><path d="M400,188H187.963v-82.23c0-36.266,30.505-65.77,68-65.77s68,29.504,68,65.77V144h40v-38.23 c0-58.322-48.449-105.77-108-105.77c-59.551,0-108,47.448-108,105.77V188H112c-33.084,0-60,26.916-60,60v204 c0,33.084,26.916,60,60,60h288c33.084,0,60-26.916,60-60V248C460,214.916,433.084,188,400,188z M420,452c0,11.028-8.972,20-20,20 H112c-11.028,0-20-8.972-20-20V248c0-11.028,8.972-20,20-20h288c11.028,0,20,8.972,20,20V452z"/></svg>
            </button>
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('company.users.tooltip.activateUser')" @click="activateUser(user.id)" v-if="!user.activated">
            ...
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('company.users.tooltip.delete')" @click="openConfirmDialog(user)" v-if="$auth('User', currentUser, user).canDelete() && !user.activated">
