      ...
      <tbody>
        <tr v-for="user in filterByEmail(email)" :class="{ 'deactivated': !user.activated }">
          ...
          <td>
            ...
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('company.users.tooltip.deactivateUser')" @click="deactivateUser(user.id)" v-if="$auth('User', currentUser, user).canDeactivate() && user.activated">
