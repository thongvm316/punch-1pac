<template>
  <setting-layout sidebar-type="company" :title="$t('company.title')" :subtitle="$t('company.users.title')">
      ...
      <tbody>
        <tr v-for="user in filterByEmail(searchText)" :key="user.id" :class="{ 'deactivated': !user.activated }">
          ...
          <td>
            ...
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('company.users.tooltip.deactivateUser')" @click="deactivateUser(user.id)" v-if="$auth('User', currentUser, user).canLockUnlockAccount() && user.activated">
    ...
    <modal :title="$t('company.users.modal.editTitle')" :modal-open.sync="isEditModalOpen">
      <user-profile :target-user="selectedUser" object-type="company" @afterUserProfileUpdated="isEditModalOpen = false" v-show="selectedUser"/>
    </modal>
  </setting-layout>
</template>
