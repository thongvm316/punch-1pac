<template>
  <main-layout :title="$t('requests.title')">
    ...
    <table class="table bg-light mt-4">
      <thead>
        <tr>
          <th class="w-cell-date">{{ $t('tableHeader.date') }}</th>
      ...
      <tbody>
        <tr v-for="request in requests" :key="request.id">
          ...
          <td class="w-cell-action">
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('tooltip.request.edit')" @click="toggleEditModal(request)" v-if="request.status === 'pending'">
            ...
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('tooltip.request.delete')" @click="openConfirmDialog(request)" v-if="request.status === 'pending'">
