<template>
  <setting-layout sidebar-type="company" :title="$t('company.title')" :subtitle="$t('company.holidays.title')">
    <div class="input-group mt-5">
      ...
      <button ref="importHolidayBtn" class="btn input-group-btn" @click="importHolidays()">{{ $t('button.common.import') }}</button>
    </div>
    ...
    <div class="toolbar clearfix mt-5">
      ...
      <button ref="btnAddHoliday" type="button" class="btn btn-success float-right" @click="toggleAddModal()">
        {{ $t('button.common.add') }}
      </button>
    </div>
    ...
    <table class="table bg-light mt-5">
      ...
      <tbody>
        <tr v-for="holiday in filterHolidays(name)" :key="holiday.id">
          ...
          <td>
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('tooltip.holiday.edit')" @click="toggleUpdateModal(holiday)">
            ...
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('tooltip.holiday.delete')" @click="localDeleteHoliday(holiday)">
    ...
    <modal :title="$t('modal.holiday.addTitle')" :modal-open.sync="isAddModalOpen">
      <holiday-form v-if="isAddModalOpen" @afterModify="isAddModalOpen = false"></holiday-form>
    </modal>

    <modal :title="$t('modal.holiday.editTitle')" :modal-open.sync="isEditModalOpen">
