<template>
  <main-layout :title="group.name">
    ...
    <div class="input-group mt-5" v-if="$auth('Group', currentUser, group.id).canAddUser()">
      <filter-user-box :queryParams="{ not_in_group_id: $route.params.id, per_page: 1000, exclude_user_ids: usersInGroup.map(user => user.id) }" :placeholder="$t('placeholder.searchByNameEmail')" :user.sync="selectedUser"/>
      <button type="button" class="btn input-group-btn" @click="localAddGroupUser">{{ $t('button.common.add') }}</button>
    </div>
    ...
    <div class="toolbar mt-5 clearfix" v-if="$auth('Group', currentUser, group.id).canEdit()">
      ...
      <div class="float-right">
        ...
        <button type="button" class="btn btn-success" @click="toggleEditModal">{{ $t('button.group.edit') }}</button>
      </div>
    </div>
    ...
    <table class="table bg-light mt-5" v-show="isShowComponent">
      ...
      <tbody>
        <tr v-for="user in filterUsers(searchText)" :key="user.id" :class="{ 'deactivated': !user.activated }">
          ...
          <td>
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('tooltip.user.edit')" @click="toggleAddModal(user)" v-if="$auth('User', currentUser, user).canEdit()">
            ...
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('tooltip.user.deactivate')" @click="openConfirmDialog(user)" v-if="$auth('User', currentUser, user).canLockUnlockAccount() && user.activated">
            ...
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('tooltip.user.activate')" @click="activateGroupUser(user.id)" v-if="$auth('User', currentUser, user).canLockUnlockAccount() && !user.activated">
            ...
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('tooltip.user.remove')" @click="openRemoveUserConfirmDialog(user)" v-if="$auth('Group', currentUser, group.id).canRemoveUser()">
    ...
    <modal :title="$t('modal.user.editTitle')" :modal-open.sync="isAddModalOpen">
      <user-profile :target-user="editUser" object-type="group" @afterUserProfileUpdated="isAddModalOpen = false" v-if="editUser"/>
    </modal>

    <modal :title="$t('modal.group.editTitle')" :modal-open.sync="isEditModalOpen">
