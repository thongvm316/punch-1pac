<template>
  <setting-layout sidebar-type="company" :title="$t('company.title')" :subtitle="$t('company.allowedIPs.title')">
    ...
    <table class="table bg-light mt-5">
      ...
      <tbody>
        <tr v-for="ip in allowedIPs" :key="ip.id">
          ...
          <td>
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('tooltip.allowedIPs.edit')" @click="toggleEditModal(ip)">
            ...
            <button class="btn btn-action btn-link tooltip" :data-tooltip="$t('tooltip.allowedIPs.delete')" @click="localDeleteIP(ip)">
    ...
    <modal :title="$t('modal.allowedIPs.addTitle')" :modal-open.sync="isAddModalOpen">
      <allowed-ip-form v-if="isAddModalOpen" @afterModify="isAddModalOpen = false"></allowed-ip-form>
    </modal>

    <modal :title="$t('modal.allowedIPs.editTitle')" :modal-open.sync="isEditModalOpen">
    ...
    <confirm-dialog
      ...
      :deleteObject="deleteIP"
      :objectId="targetIp.id"
      >
